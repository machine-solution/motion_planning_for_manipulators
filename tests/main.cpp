...
TEST_CASE("JointState arithmetic")
{
    ...
    JointState b({3, -4});
    a += b;
    CHECK(a == JointState({4, -2}));
    b += JointState({1, 0});
    CHECK(b == JointState({4, -4}));
    CHECK(a != b);
    CHECK(JointState{4, 0} + JointState{0, -4} == b);
    b += a;
    CHECK(JointState{4, -4} + a == b);
}
...
TEST_CASE("File read test")
{
    ...
    testReadFile(2, "tests/samples/load_taskset/2-dof_pos_test_3.scen", 5, TASK_POSITION);
